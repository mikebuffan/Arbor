...
## Engineering status
The draft broker file at `apps/backend/lib/grove/privateReadBroker.ts` is now wired in source to `apps/backend/app/api/grove/ark/status/route.ts` and the private Flutter reader selects `/api/grove/ark/status`, while original Firefly selects `/api/ark/status`. Twenty synthetic backend cases and one Flutter route regression case have been committed. **Exact code head `a39dd78123f8b80d4981c2fd56c079e482e17501` passed GitHub Actions [Grove verification run 35687588013](https://github.com/mikebuffan/Arbor/actions/runs/35687588013): backend tests, TypeScript, backend build, Flutter analyze and Flutter tests all green.** Previous head failed the new typecheck due to a database row type cast; this was corrected before the green run. A private-host isolation guard now rejects legacy Firefly APIs, attachment writes, public home and scheduled heartbeat whenever `GROVE_API_ENABLED=true`, while preserving ordinary Firefly behavior when unset. Five additional synthetic guard tests were added. **Exact updated code head `9f363f989ed42dbc66dee4eeda5cf37f0c371787` passed [verification run 35687923776](https://github.com/mikebuffan/Arbor/actions/runs/35687923776): backend tests, TypeScript, backend build, Flutter analysis and Flutter tests all green.** The configured Vercel backend source still has a heartbeat cron declaration; do not create a separate Grove deployment before reviewing that cron and environment isolation. **No protected Grove API deployment, configured credentials, hosted LM, invited owner or real private→Firefly read has been verified.** The previous connector route/Flutter write block was resolved by retrying smaller, targeted writes; do not repeat that task manually.
