...
- Optional *Grove Vercel project-only* branch policy is **source-tested** in `ops/grove/should-build-private-host.mjs` (Vercel exit 0 skips, 1 builds): old confirmed production branch only when target is production and this candidate only when preview. Unrelated research/public/main/old drafts skip. **IMPORTANT: old production branch may NOT contain this helper! Do not install `node ../../ops/grove/should-build-private-host.mjs` as a project-wide command while old production tracking is retained; a production build on old code could lack it.** Owner-approved branch-independent equivalent for Grove project settings, to review rather than execute here: `sh -c 'case "$VERCEL_ENV:$VERCEL_GIT_COMMIT_REF" in production:deploy/grove-private-api-20260921|preview:release/grove-private-source-candidate-20260923) exit 1;; *) exit 0;; esac'`. Verify project settings support this command and both env vars before use, then re-test old production/preview behavior. No branch policy is installed in Vercel by this PR; do NOT apply on Firefly or research projects.
