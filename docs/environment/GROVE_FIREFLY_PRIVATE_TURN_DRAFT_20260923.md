...
## Private Grove transcript recovery endpoint (follow-up on #194)

Source code adds GET /api/grove/chat/history?projectId=UUID&conversationId=UUID on the EXISTING Grove private owner/conversation broker; independently requires GROVE_PRIVATE_CHAT_PREVIEW_ENABLED=true AND GROVE_PRIVATE_TRANSCRIPT_ENABLED=true, both default OFF. Reads only six newest **complete** pairs in the dedicated Grove-only grove_private_turns proposal, never Firefly's public messages. The endpoint rejects unknown/duplicate query parameters, verifies fresh Grove Auth+invitation+revocable mapping+project grant+Firefly conversation ownership before creating a Grove service-role transcript store, uses three-field owner/project/conversation query scope, no-store responses and sanitized errors, and does not return user IDs, internal host context, or ARK completion claims. historyMayBeTruncated means six rows may omit older context, NOT the archive is complete. Synthetic route acceptance covers OFF, malformed/foreign scope, denied owner, bounded data, truncation and provider failure without leaked text.

**Remaining phone/UI step:** Grove private Talk page intentionally still says text/voice are unavailable until owner authorization, provider deployment and host features can be verified. Add a feature-gated private chat UI on the GROVE Flutter branch only after the actual Grove-granted project and EXISTING Firefly-owned conversation selection are solved; project gate currently clears old Firefly conversation rather than inventing permission. Do not switch that UI to public Firefly /api/chat and do not fabricate an ARK-linked Firefly conversation ID. The new GET endpoint is testable backend source, not yet user-visible on Android.

**Review before enabling:** privacy retention/deletion/export consent; revocation during long model calls; rate limits and multi-instance HMAC replay protection; isolated Grove migration + owner/guest RLS trial; actual hosted private v0.3.5 LM and real auth/phone acceptance. Never auto-provision Grove owner/grant from a model or copy private text to a public repository.
