...
- **#186** is a temporary **CI-ONLY, DO NOT MERGE** draft from #185 to the existing Grove verification target. Earlier run **35803031084** failed Flutter analysis on a missing responsive-height local, corrected on both UI branches. Later run **35803136465** passed both backend jobs and analysis, but two new modal-timing widget tests failed; run **35803295119** reduced this to one off-screen staircase-menu test (156 passed, one failed). The staircase now navigates directly to Observatory, avoiding the unnecessary menu. **Latest source head `dec5e3f1b8d3b6bd847b828cdcb44284579f8d13`; CI run `35803531108` was queued at this report. Not yet a green receipt.** #186 must not be used as a canonical implementation PR.
