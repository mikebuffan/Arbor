...
### 2026-09-23 follow-up — Grove Text leave/return regression on existing #196 branch

- [x] Reconciled exact backend #194 at `8bfc519f5e7892202be8497c917cce239a528e12` with [CI-only mirror #195 run 35849039124](https://github.com/mikebuffan/Arbor/actions/runs/35849039124): backend/control builds and Flutter test/analyze/debug APK PASS. Vercel rate-limit statuses are deployment failures, not those source tests.
- [x] Reconciled separate Grove-owned phone #196 at prior `d9c80a0f5bff2511cddacf36fae050b927d0ac9c` with [CI-only mirror #197 run 35859270891](https://github.com/mikebuffan/Arbor/actions/runs/35859270891): existing picker, bounded history, signed-in host gate and Android compile PASS. No physical owner/device or real-Grove acceptance.
- [x] Advance 06.03–06.08 **source only** on existing Grove #196, without copying Flutter files into this backend branch. Source now refuses to clear a sent draft/retry ID until a follow-up scoped history read contains the exact persisted request ID, user text and assistant text. Tests exercise stale read → same request replay, actual Text-widget leave/remount → existing stored answer, empty picker and account scope change. Fixture formerly compared `projectId` to itself; it now checks the expected scoped values.
- [ ] **Exact-head new phone CI:** #196/#197 head `7bcd7e20ba5eb20836481d114adfcd3c26776447`, [run 35865685238](https://github.com/mikebuffan/Arbor/actions/runs/35865685238) was QUEUED when this checkpoint was authored. Do not promote to VERIFIED-CI until all required jobs finish successfully.
- [ ] **Still owner/release-gated:** no active real Grove invitation/account bridge/project grant verified in this checkpoint; transcript SQL remains only proposed; no newly created Firefly conversation workflow; feature remains default OFF; no signed private model host or physical leave/reopen proof. Source tests use synthetic fake server storage and do not establish live persistence. No merge, deploy, paid service, owner grant or migration performed.

### Parallel Epstein research-engine integration boundary — September 23, 2026

- [x] Reconcile research lane's distinct canonical 64-item checklist and handoff, `docs/research/EPSTEIN_RESEARCH_ENGINE_HANDOFF_20260923_RUN4.md`, research #199 (child #189 → #187 → #181). Grove work MUST NOT implement a second evidence engine, source-ingest worker, checkpoint queue or assertion graph.
- [x] Identify integration ownership: Grove/ARK/Layer private chat remains #194 plus Grove phone #196; research owns #199 and its descendants; independent LM is separate. This cross-lane checklist owns dependency tracking and synthetic interface acceptance ONLY.
- [ ] Research item 45: obtain successful exact-head disposable PostgreSQL CI on #199 before treating retry/failure/stalled-lease semantics as verified; then item 52 deterministic persisted simulation. Never use a main-base CI bridge that would initiate an unapproved deployment.
- [ ] GROVE↔RESEARCH seam: inspect EXISTING owner/project-scoped ARK read and research session/checkpoint interfaces; design/test read-only metadata handoff of authorized research objective, checkpoint, pending work and provenance REF IDs into Grove's existing continuity/context. Preserve source-family independence, identity-review HOLD, privacy redactions, contradiction flags and explicit uncertainty; do not copy source bodies, investigation private data or secrets into model prompts or GitHub.
- [ ] Fixture: independently scoped research project R and personal project P; interrupt/reopen each, verify separate conversation/project selection, unchanged research checkpoint, latest authorized evidence refs, no B/Q cross-owner leak, no model-prose-as-investigation-receipt, no execution or publication. Existing ARK Preview research permissions must not be interpreted as Grove private owner/grant.
- [ ] Explicitly HOLD real research source ingestion, live worker/scheduler, paid inference, production migrations, merges, deployment, unreviewed publication and private/victim data processing pending separate approval. The research lane must not modify #194/#196 or their integration queue; reconcile latest heads before touching shared files.
