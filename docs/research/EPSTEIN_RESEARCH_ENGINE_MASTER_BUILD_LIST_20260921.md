...
7. [~] #152 pins Debian amd64 base by digest and exact Poppler package version; exact-head CI must prove the image builds before this becomes [x].
...
18. [ ] **BLOCKED — sandbox verification gate:** independently published benign PDF acceptance + manual rendered-page/line-order verification must wait for #152 exact-head sandbox CI success and review.
...
21. [~] #152 implements the executable disposable renderer boundary specified by #135: digest-pinned amd64 Debian base, exact Poppler package, numeric non-root user, immutable `sha256:` image-ID requirement, no network, read-only root, all capabilities dropped, no-new-privileges, bounded RAM/swap/PIDs/CPU/file descriptors/tmpfs/time/output, explicit read-only input and output-only bind. CI renders only a generated harmless synthetic PDF. **Pending exact-head CI; not yet authorized for external PDFs.**
22. [ ] Opt-in OCR processor with source-image reference, confidence and mandatory human verification. Depends on verified item 21 for untrusted inputs.
...
48. [ ] **BLOCKED — verified sandbox item 21:** bounded authorized capture/parse executor.
...
58. [ ] **BLOCKED — verified executable sandbox + source authorization:** verify physical pages/images of specific public EFTA records against original OIG report.
...
**Verified research lineage:** #139 exact head `9699ce8fee510979ffc9909220752f49a968a083` passed Arbor Integration CI run 35663024929; #141 exact head `a8909b880d4d4ab107244f165a20a51d593b7024` passed configured CI; #142 exact head `d623dbc7281f518b74f0330e63228fdf92b7999d` passed configured CI; #144 implementation head `e0805c884a57c79b1d158cafacced7ffd1eb4a97` passed Arbor Integration CI run 35672651469. Current #144 docs head is `fec26c3e6006a895e04042b47ed97167e220c227`.

**#152 implementation:** branch `feat/ark-pdf-disposable-sandbox-20260921`, parent #144 head `fec26c3e6006a895e04042b47ed97167e220c227`; implementation head before this handoff was `941d809f429a7ddc22882165338cc5c9ba73798c`. It adds the executable isolation boundary and a real Poppler render of a harmless generated PDF in CI. CI was queued when this handoff was written, so item 21 remains [~], not [x]. No external/untrusted PDF has been executed.

**Next numbered item:** finish verification of 21 by inspecting #152 exact-head CI and correcting any sandbox failures. Only after that succeeds may item 18 move to a separately reviewed independently published benign PDF acceptance/manual visual check. Items 40–45 remain BLOCKED pending explicit disposable-DB/cost approval; live worker, scheduler, deployment, real EFTA processing, private data and publication remain separately gated.
