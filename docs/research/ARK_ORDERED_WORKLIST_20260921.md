...
- [~] Added a standards-conformant *synthetic harmless* four-page PDF fixture parsed by real Poppler in #134; it exercises original bytes/hash, source URL, document ID and physical PDF pages. Independently published benign original PDF with manual page-image review remains OPEN.
...
- [~] Added a pure exact-passage bridge (`pageObservation.ts`) from a validated text-layer PDF page to a source locator, actual captured byte hash, physical page, optional printed folio and UTF-16 start/end offset. It rejects unsupported/scanned pages and substituted text/hash, and retains independent-image/privacy HOLD. **Still needed:** actual PDF parser, persisted atomic evidence write, separate reviewed interpretive/epistemic claims and page-image alignment; this bridge is NOT a live extractor.
...
## Integration checkpoint — exact quote observations (isolated child branch)\n\n- [x] Added `pageObservation.ts` and focused unit tests for exact source-backed excerpts, position, physical page, printed folio, unchanged file hash, text substitutions, image-only/failed pages and HOLD statuses. Source-backed quote text is kept separate from any interpretive claim.\n- [x] Found that `apps/backend/vitest.config.ts` previously matched only `lib/**/__tests__/**/*.test.ts`, which excludes existing `lib/research/*.test.ts`. Child branch now explicitly includes the research suite. Previous backend-green CI on parent did not itself prove those research test files executed.\n- [ ] Verify actual research test execution and complete backend/build/Flutter CI for the final child branch head; run real original public PDF fixture separately.\n- [ ] Do not merge this child work independently alongside a separately rebased parent; preserve one source-of-truth branch and review the parent's movement before integration.\n\n## Implementation checkpoint — local Poppler parser and actual PDF→evidence fixture\n\n- [x] Child PR #134 reuses #123 original-byte and page-record contracts and #131 exact-passages bridge. It does NOT create another engine.\n- [x] Established Poppler local CLI does real metadata, page-by-page text and image inventory on a valid synthetic four-page PDF, with time/file/page/output limits, 0600 temporary file and cleanup.\n- [x] Genuine fixture integration tests carry its SHA-256 and physical page through a selected passage into a comparison draft on HOLD.\n- [x] No-text image page is image_only; blank page is extraction_failed/not proof of absence; malformed PDF and HTML fail closed.\n- [ ] Verify latest exact-head CI after this worklist/doc update.\n- [ ] Second independently published benign original PDF with manual page-image verification and sandbox threat model.\n- [ ] See `docs/research/EPSTEIN_RESEARCH_ENGINE_MASTER_BUILD_LIST_20260921.md` for full deduplicated dependency order and release gates.\n\n## Implementation checkpoint — source-byte and lease safeguards
...
- [~] A **real sandbox-only Poppler parser** in #134 reads actual PDF bytes through pdfinfo, pdftotext and pdfimages and produces page records. Tests prove text layer, image-only and blank/unclassified status; original independent public PDF fixture, page-image check and OCR are still required. Parser is NOT in the live worker or scheduler.
