...
## Disposable PostgreSQL first observed CI run and repair

- GitHub Actions run 35804531241 reached the isolated PostgreSQL service. PostgreSQL **17.11** initialized; synthetic auth/project fixture and the complete proposed research SQL (tables, indexes, RLS policies, RPCs and grants) executed successfully. The runner removed the service container/network during job cleanup.
- The first acceptance script stopped at line 82 with `syntax error at or near "$"` in the **test script's grant assertion**; the earlier synthetic claim, settle and duplicate-receipt assertion had executed as a successful DO block. This is NOT proof that the whole DB acceptance passed.
- Corrected `do $ ... end $;` to `do $$ ... end $$;` in `ops/research/disposable-db/10-acceptance.sql`, commit `059d41ee302c7d25822473517e4db8775caaadca`. Await a new full CI run before marking any remaining checks PASS.
- Added separate-connection claim-vs-claim and duplicate-settle-vs-duplicate-settle race harness `30-concurrency.sh`. This does not yet cover a controlled lock wait across the deadline or simultaneous STOP-versus-settle; keep those pending.
