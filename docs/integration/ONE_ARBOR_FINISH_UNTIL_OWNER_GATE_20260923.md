...
- [ ] Verify dedicated private Vercel **project ID and deployed exact SHA**. Prior owner UI reported project creation, while connected project listing returned only Firefly; this is inconclusive inventory, NOT proof that Grove doesn't exist. Do not create a duplicate or deploy to Firefly. Review branch, `apps/backend` root, origin, zero cron, no OpenAI runtime dependency.
