...
Earlier Supabase listing had current Firefly, ARK Preview and an inactive older project; the most recent read lists only **Firefly and ARK Preview**, with **no dedicated Grove project**. Aurixus Studios organization now reports **Pro**, not Free. Supabase quoted an **additional $10 monthly** for a new Grove project under Aurixus; owner approval of the exact charge, organization and region remains necessary before creation. Connected Vercel previously listed only the existing `firefly` project, not a distinct Grove API project. These are account-visible inventory observations, not a claim that no other inaccessible provider exists. No project, account, key, deployment, billing or migration was created or modified by this inventory read.
