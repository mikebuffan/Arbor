...
**CRITICAL same-state correction, verified by live read-only counts on September 24:** The installed ChatGPT ARK connector currently points to **ARK Preview**, not the existing **Firefly** state that Grove's approved private bridge design targets. Firefly DB: 5 projects, 1 ARK objective, 1 task, 2 runtime-state records, 0 checkpoints. Preview DB: 2 objectives, 3 tasks, 0 runtime-state records, 0 checkpoints (the ChatGPT owner's scoped read correctly returned only the owned completed smoke objective). Grove DB: owner grants = 0, identity bridges = 0, ARK project grants = 0. Do NOT call the Preview-only ChatGPT readback proof of ChatGPT↔Grove same-state acceptance. Before production acceptance, choose the canonical live Firefly-side owned project, provision the separately reviewed/revocable Grove bridge for that owner, and connect ChatGPT's live app securely to the SAME canonical store while retaining ARK Preview isolated. No bulk data copy, cross-issuer token acceptance or automatic default grant.
...
4. [ ] **Canonical state + Grove owner:** use the existing Firefly-side live state for eventual private operation; ensure ChatGPT production connector and Grove broker resolve the SAME authorized owned project (not Preview vs Firefly). Pin the correct Grove source head (closed #211 is NOT a release), confirm existing isolated Vercel project/team/deploy SHA via owner who can access it; verify private Supabase account invitation, revocable project bridge and Grove API read of the SAME owned ARK state. Cross-provider auth must happen server-side; no Firefly JWT accepted as a Grove JWT.
