...
## Second experiment: the source-assertion relationship graph

`associativeRelationGraph.ts` adds the missing **structural** ingredient on the *same isolated branch*: a typed edge `(subjectId, has_nickname, nicknameId)` with source receipt(s), current `asserted|contested` status, owner/project scope, and routing strength explicitly separated from truth. `my`/`your` referents are resolved from **trusted caller-provided** speaker/addressee IDs; the graph does not infer them from text or become a natural-language model. A second speaker can make a `your nickname` assertion about the SAME underlying person and the same edge is reinforced exactly once per new receipt. A contested edge stops appearing as active but retains its source history; another trusted review is required to restore it. Identical nicknames may belong to more than one person: no universal exclusivity rule. This is independent of any real private identity or real ARK memory.

Local strict TypeScript compiled and an 11-assertion isolated Node smoke confirmed speaker/addressee transfer, source preservation, idempotency, conflicting receipt rejection, scope denial, nonexclusive nicknames, contested-edge HOLD, and `grantsExecution:false`. Full committed Vitest and exact-head CI must still be checked. **The graph is hand-specified symbolic relation storage, not learned relation extraction**; future work tests learning to propose edges from language with independent verification and well-designed held-out scenarios.