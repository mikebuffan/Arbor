-- DRAFT / ISOLATED PR ONLY. Do not apply in production before sandbox DB tests.
...
begin
  ...
  if p_status not in ('completed','checkpointed','blocked','failed')
     ...
     or (p_status='completed'
          and p_unresolved_required_work<v_session.unresolved_required_work
  ...
  update public.arbor_research_sessions
    set consumed_work_units=consumed_work_units+
        ...
        completed_evidence_refs=(select coalesce(array_agg(distinct ref order by ref),'{}'::text[])
          from unnest(v_session.completed_evidence_refs||p_evidence_refs) as ref
