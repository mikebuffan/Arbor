...
type WorkRow = {
  ...
  title: string;
  problem_key: string;
  status: ArborWorkState["status"];
  hypothesis: string | null;
  hypothesis_confidence: number | null;
  current_goal: string;
  next_action: string | null;
  evidence: ArborWorkState["evidence"] | null;
...
function rowToState(row: WorkRow): ArborWorkState {
  return {
    id: row.id,
...
export async function listActiveWork(params: {
  ...
  const { supabase, userId, projectId, limit = 10 } = params;
  ...
  return ((data ?? []) as WorkRow[]).map(rowToState);
}

export async function listActiveWorkSafe(params: {
  supabase: SupabaseClient;
  userId: string;
  projectId: string;
  limit?: number;
...
export async function upsertWorkState(params: {
  supabase: SupabaseClient;
  userId: string;
  ...
  return rowToState(data as WorkRow);
}
