...
import { getMemoryContext, isMemoryInProjectScope } from "@/lib/memory/retrieval";

describe("memory project isolation", () => {
  beforeEach(() => { vi.clearAllMocks(); });

  it("looks up an existing memory by authenticated user, project, and key", async () => {
    const query = { select: vi.fn(), eq: vi.fn(), is: vi.fn(), maybeSingle: vi.fn().mockResolvedValue({ data: null, error: null }) };
    ...
    await reinforceMemoryUse("user-a", ["shared-key"], "project-a", supabase);
  ...
  it("filters vector retrieval results to the authenticated project", () => {
    expect(isMemoryInProjectScope({ project_id: "project-a", scope: "project" }, "project-a")).toBe(true);
    expect(isMemoryInProjectScope({ project_id: "project-b", scope: "project" }, "project-a")).toBe(false);
    expect(isMemoryInProjectScope({ project_id: null, scope: "global" }, "project-a")).toBe(true);
    expect(isMemoryInProjectScope({ project_id: "project-a", conversation_id: "conversation-a", scope: "conversation" }, "project-a", "conversation-a")).toBe(true);
  ...
  it("requires controlled imports to create projects with the supplied user_id", () => {
    const source = fs.readFileSync(path.resolve(process.cwd(), "scripts/import_chatgpt/runImport.ts"), "utf8");
  ...
  it("keeps retrieval fresh and does not log the current user message", () => {
    const source = fs.readFileSync(path.resolve(process.cwd(), "lib/memory/retrieval.ts"), "utf8");
    expect(source).not.toContain("const memoryCache");
    expect(source).not.toMatch(/console\.(log|debug)\([^)]*latestUserText[\s\S]*?\)/);
  ...
  it("retrieves the corrected value and excludes superseded stale aliases", async () => {
    ...
    query.select.mockReturnValue(query); query.eq.mockReturnValue(query); query.is.mockReturnValue(query); query.order.mockReturnValue(query); query.or.mockReturnValue(query); query.limit.mockReturnValue(query);
    const supabase = { from: vi.fn().mockReturnValue(query) } as unknown as SupabaseClient;
    const result = await getMemoryContext({ supabase, authedUserId: "user-a", projectId: "project-a", latestUserText: "What is the observatory access phrase?" });
    expect(result.keysUsed).toEqual(["project.observatory.access_phrase"]);
