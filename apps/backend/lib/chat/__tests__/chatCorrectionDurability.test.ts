...
vi.mock("@/lib/memory/correctionResolution", async (importOriginal) => ({
  ...(await importOriginal<typeof import("@/lib/memory/correctionResolution")>()),
...
vi.mock("@/lib/memory/consolidateCandidates", () => ({ consolidateMemoryCandidates: mocks.consolidateMemoryCandidates }));
vi.mock("@/lib/memory/promoteCandidates", () => ({ promoteEligibleMemoryCandidates: mocks.promoteEligibleMemoryCandidates }));
...
vi.mock("@/lib/chat/turnPersistence", async (importOriginal) => ({
  ...(await importOriginal<typeof import("@/lib/chat/turnPersistence")>()),
...
function request() {
  return new Request("https://arbor.test/api/chat", {
    ...
    body: JSON.stringify({ projectId: PROJECT_ID, turnId: TURN_ID, userText: USER_TEXT }),
  });
}
...
function resolvedCorrection() {
  return {
    ...
    resolution: {
      ...
      canonical: { id: MEMORY_ID, key: "project.observatory.access_phrase" },
      staleAliases: [],
    },
...
describe("explicit correction request-path durability", () => {
  ...
  beforeEach(() => {
    ...
    mocks.blockAgencySession.mockImplementation(async ({ agency, blocker, unresolvedWork }) => ({
      ...agency,
      status: "blocked" as const,
      blocker,
      unresolvedWork,
    }));
    mocks.completeAgencySession.mockImplementation(async ({ agency, verified }) => ({
      ...agency,
    ...
    mocks.timelineCreate.mockResolvedValue({ record: mocks.timelineRecord });
    ...
    mocks.consolidateMemoryCandidates.mockResolvedValue(undefined);
    mocks.promoteEligibleMemoryCandidates.mockResolvedValue(undefined);
  ...
  it("does not make success returnable until the correction is durable", async () => {
    ...
    expect(await response.json()).toMatchObject({ ok: true, assistantText: "Understood: Amber Quill." });
    expect(durableValue).toBe("Amber Quill");
    expect([...turnStore.messages.values()].filter(({ role }) => role === "assistant")).toHaveLength(1);
  ...
  it("fails conservatively before assistant persistence when resolution is unavailable", async () => {
    mocks.persistClassifiedMemoryTurn.mockResolvedValue({
      ...resolvedCorrection(),
      resolution: { status: "not_found", canonical: null, staleAliases: [] },
      corrected: null,
    });
    const response = await POST(request());
    expect(response.status).toBe(409);
    expect(await response.json()).toEqual({ ok: false, error: "correction_unresolved" });
    expect([...turnStore.messages.values()].filter(({ role }) => role === "assistant")).toHaveLength(0);
    expect(mocks.scheduleChatPostResponseWork).not.toHaveBeenCalled();
  });
  ...
  it("does not increment again when retry follows a post-correction assistant insert failure", async () => {
    ...
    mocks.persistClassifiedMemoryTurn.mockImplementation(async () => {
      ...
      return {
        ...resolvedCorrection(),
        resolution: { ...resolvedCorrection().resolution, status: "already_applied" as const },
      };
    });
    ...
    const retry = await POST(request());
    expect(retry.status).toBe(200);
    expect(await retry.json()).toMatchObject({ ok: true, assistantText: "Understood: Amber Quill." });
    expect(correctionCount).toBe(1);
