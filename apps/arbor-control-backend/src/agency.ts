  ...
  for (let round = 0; round < maxRounds; round += 1) {
    ...
    if (calls.length) {
      ...
      for (const call of calls) {
        ...
        // Recovery discovery is intentionally opt-in. A different capability
        // may require a different argument shape, so never replay the failed
        // call's arguments into arbitrary tools. Registered recovery routes
        // must prove their own compatibility before execution.
