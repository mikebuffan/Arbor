...
export function reconcileSelfModelClaims(state: ArborState): ArborState {
  ...
  for (const summary of summaries) {
    ...
    const status: SelfModelClaimStatus =
      summary.status === "candidate" ? "candidate" :
      summary.status === "contested" ? "contested" : "insufficient";
    ...
    next.push({
      ...
      supportDomains: summary.supportDomains,
      contradictionDomains: summary.contradictionDomains,
      evidenceDigest,
...
export function renderSelfModelClaimProjection(state: ArborState): string {
  ...
  return [
    "ARBOR FALSIFIABLE SELF-MODEL CLAIMS",
    "Claims are inferences from recorded behavior, not facts merely because they were stated by Arbor or Danelle.",
    "Candidate claims remain revisable; contradictory evidence contests them; superseded claims remain in history.",
    ...active.map((claim) => `- ${claim.key} | status=${claim.status} | confidence=${claim.confidence} | support=${claim.supportCount} | contradict=${claim.contradictionCount} | domains=${claim.supportDomains.join(",") || "none"}`),
  ].join("\n");
}

function claimConfidence(support: number, contradiction: number, average: number, domains: number): number {
  const evidenceWeight = Math.min(1, support / 4);
  const domainWeight = Math.min(1, domains / 3);
  const contradictionPenalty = Math.min(0.75, contradiction * 0.25);
  return round(Math.max(0, Math.min(1, average * 0.5 + evidenceWeight * 0.3 + domainWeight * 0.2 - contradictionPenalty)));
}
