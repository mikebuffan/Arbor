...
export class CogMoleculeRuntime {
  private readonly configuredMaxRounds: number; private readonly releaseFriction: number; private readonly frictionWeights: FrictionWeights;
  constructor(private readonly options: CogMoleculeRuntimeOptions) { if (!options.cogs.length) throw new Error("cog_molecule_requires_cogs"); this.configuredMaxRounds = options.maxRounds ?? 32; this.releaseFriction = options.releaseFriction ?? 0.05; this.frictionWeights = { ...DEFAULT_WEIGHTS, ...options.frictionWeights }; }
  async run(initial: CogPacket): Promise<MoleculeResult> {
    let packet = normalizePacket(structuredClone(initial)); packet.friction = this.measureFriction(packet);
  ...
  private measureFriction(packet: CogPacket): number { const unresolvedPressure = saturating(packet.unresolved.length); const contradictionPressure = saturating(packet.hypotheses.reduce((sum, h) => sum + h.contradictions.length, 0)); const challengePressure = saturating(unresolvedChallenges(packet)); const uncertaintyPressure = packet.hypotheses.length ? packet.hypotheses.reduce((sum, h) => sum + (1 - clamp01(h.confidence)), 0) / packet.hypotheses.length : 0; return clamp01(unresolvedPressure * this.frictionWeights.unresolved + contradictionPressure * this.frictionWeights.contradiction + challengePressure * this.frictionWeights.challenge + uncertaintyPressure * this.frictionWeights.uncertainty); }
}

function normalizePacket(packet: CogPacket): CogPacket {
  const evidence = (packet.evidence ?? []).map((value) => ({ ...value, provenance: canonicalStrings(value.provenance) }));
  const hypotheses = (packet.hypotheses ?? []).map((value) => ({ ...value, support: canonicalStrings(value.support), contradictions: canonicalStrings(value.contradictions) }));
  const challenges = (packet.challenges ?? []).map((value) => ({ ...value, provenance: canonicalStrings(value.provenance) }));
  return { ...packet, evidence: canonicalObjects(dedupeExact(evidence)), hypotheses: canonicalObjects(dedupeExact(hypotheses)), unresolved: canonicalStrings(packet.unresolved ?? []), challenges: canonicalObjects(dedupeExact(challenges)), provenance: canonicalStrings(packet.provenance ?? []) };
}
function dedupeExact<T extends CogEvidence | CogHypothesis | CogChallenge>(values: T[]): T[] { const seen = new Set<string>(); return values.filter((value) => { const key = JSON.stringify(value); if (seen.has(key)) return false; seen.add(key); return true; }); }
function canonicalObjects<T>(values: T[]): T[] { return [...values].sort((a, b) => JSON.stringify(a).localeCompare(JSON.stringify(b))); }
function canonicalStrings(values: string[]): string[] { return [...new Set(values.map((value) => value.trim()).filter(Boolean))].sort(); }
